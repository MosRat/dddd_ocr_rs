...
impl Rec {
    ...
    fn preprocess(img: &DynamicImage) -> DDDDOcrResult<ArrayBase<OwnedRepr<f32>, Dim<[usize; 4]>>> {
        ...
        let mut input = Array::zeros((1, 1, 64, (w * 64 / h) as usize));
        img.enumerate_pixels().for_each(|(x, y, &Luma([pixel]))| {
            // eprintln!("{x},{y}:{}",(((pixel as f32) / 255.) - 0.5) / 0.5);
    ...
    fn compose_gif<R: Read>(img: R) -> DDDDOcrResult<DynamicImage> {
        ...
        let mut decoder = decoder.read_info(img)?;
        ...
        // 创建一个空的合成图像
        let mut composite_image = RgbaImage::new(new_width, new_height);
        let mut tmp: Vec<_> = Vec::with_capacity(6);
        let mut count: u32 = 0;

        while let Some(frame) = decoder.read_next_frame()? {
            let frame_image = image::imageops::resize(&RgbaImage::from_raw(width, height, frame.buffer.to_vec())
                .expect("Failed to create image from frame")
                                                      , new_width, new_height, FilterType::Lanczos3,
            );
            tmp.push(frame_image);
            count += 1;
        }
        for (x, y, pixel) in composite_image.enumerate_pixels_mut() {
            pixel.0 = [
                (tmp.iter().map(|p| p.get_pixel(x, y).0[0] as u32).sum::<u32>() / count) as u8,
                ((tmp.iter().map(|p| p.get_pixel(x, y).0[1] as u32)).sum::<u32>() / count) as u8,
                (tmp.iter().map(|p| p.get_pixel(x, y).0[2] as u32).sum::<u32>() / count) as u8,
                (tmp.iter().map(|p| p.get_pixel(x, y).0[3] as u32).sum::<u32>() / count) as u8,
            ]
        }
